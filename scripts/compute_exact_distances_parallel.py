...
for dataset in datasets:
    # full counting
    path_counts = work_dir_path+str(dataset)+"_counts_frequentexact_"+str(k)+".txt"
    path_binary = work_dir_path+str(dataset)+"_frequentexact_"+str(k)+".mf"
    path_reads = str(dataset)+"/"+str(dataset)+".fastq"
    out_path = dataset+"_"+temp_file_path
    cmd = jellyfish_path+" count -m "+str(k)+" -s 1M -t 5 -o "+str(path_binary)+" -C "+str(path_reads)+" > "+str(out_path)+" &"
    #print cmd
...
for theta in thetas:
    ...
    for dataset in datasets:
        ...
        path_reads = str(dataset)+"/"+str(dataset)+".fastq"
        out_path = dataset+"_"+temp_file_path
        cmd = jellyfish_path+" dump -o "+str(path_counts)+" -L "+str(minsupp)+" "+str(path_binary)+" > "+str(out_path)+" &"
        #print cmd
        to_run.append((cmd , dataset))
    patterns = list()
    ...
    for i in range(len(datasets)):
        for j in range(i+1 , len(datasets)):
            ...
            path_counts1 = work_dir_path+str(dataset1)+"_counts_frequentexact_"+str(k)+".txt"
            path_counts2 = work_dir_path+str(dataset2)+"_counts_frequentexact_"+str(k)+".txt"
    ...
    for dataset in results:
        ...
        output = str(dataset1)+"_exact_freq;"+str(dataset2)+"_exact_freq;"+str(bcdist)+";"
